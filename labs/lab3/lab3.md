...
- на оценку отлично также измерить major fault и minor fault во время работы второго процесса для каждого из экспериментов (так же взять среднее; major fault и minor fault доступны только на линукс, следовательно, код надо запускать там). Разумеется, быть готовым объяснить полученные результаты.
