...
- код должен запускаться на Linux
...
- нужно обязательно сравнить время работы в зависимости от количества потоков/процессов и построить сравнительные графики. Объяснить результат:
    ...
    3. Почему процессы работают медленнее потоков (если делаете обе части).
- рекомендуется выполнить не менее 3 замеров для каждого числа потоков/процессов и усреднить результат
...
- если вы создаете процессы, то их *обязательно создавать через fork*
...
- до защиты подготовить графики, чтобы было, что обсуждать 
...
- если запускаете код на виртуальной машине, убедитесть, что у вас выделено больше чем 1 ядро CPU для этой машины. Иначе эффект ускорения может быть незначительный или не быть вообще
